qtl.heatmap = function(lod, ...) {
  ...
  lod = exp(lod)
  lod = lod / matrix(apply(lod, 2, max, na.rm = T), nrow(lod), ncol(lod),
                     byrow = T)
  ...
  plot(dend, horiz = T, axes = F, yaxs = "i", leaflab = "none")
  par(las = 1, plt = c(0, 0.8, 0.1, 0.9))
  breaks = 0:100 / 100
  col = colorRampPalette(c(gray(10:2/10), rgb(1,0.5,0), rgb(1,0,0)))(length(breaks) - 1)
  image(1:nrow(lod), 1:ncol(lod), lod, breaks = breaks, col = col, ann = F,
        axes = F, ...)
