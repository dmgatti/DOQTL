scanone.perm = function(pheno, pheno.col = 1, probs, K, addcovar, intcovar, snps,
               path = ".", nperm = 1000) {
  ...
  if(!missing(addcovar)) {
    ...
    addcovar = addcovar[rowMeans(is.na(addcovar)) == 0,,drop = F]
  ...
  if(!missing(intcovar)) {
    ...
    intcovar = intcovar[rowMeans(is.na(intcovar)) == 0,,drop = F]
  ...
  for(i in pheno.col) {
    ...
    if(!missing(addcovar)) {
      perms = permutations.qtl.LRS(pheno = p[keep], prob = probs[keep,,], 
              snps = snps, addcovar = addcovar[keep,,drop=F], nperm = nperm)
    } else {
      perms = permutations.qtl.LRS(pheno = p[keep], prob = probs[keep,,], 
