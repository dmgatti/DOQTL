  ...
  if(length(missing) > 0) {
    ...
    if(mthd == "allele") {
      ...
      founders$geno = data.frame(rbind(founders$geno, matrix("", length(missing), 
                      ...
                      colnames(founders$geno)))), check.names = F)
      ...
      for(i in seq(1, length(missing), 2)) {
        ...
        p1g = lapply(founders$geno[founders$code == par[1,i], curr.snps, drop = F],
              table)
        p2g = lapply(founders$geno[founders$code == par[2,i], curr.snps, drop = F],
              table)
      ...
      if(length(curr.snps) > 0) {
        # Females
        for(i in seq(1, length(missing), 2)) {
          geno.rows = which(ounders$code == missing[i] & founders$sex == "F")
          p1g = lapply(founders$geno[founders$code == par[1,i], curr.snps, drop = F],
                table)
          p2g = lapply(founders$geno[founders$code == par[2,i], curr.snps, drop = F],
                table)
        ...
        for(i in seq(1, length(missing), 2)) {
          ...
          founders$geno[geno.rows, curr.snps, drop = F] = "N"
        } # for(i)
      } # if(length(curr.snps) > 0)
    ...
    # Intensities
      ...
      for(i in 1:length(unique.par)) {
        ...
        par.means$x[i,] = colMeans(founders$x[rows,,drop = F], na.rm = T)
        par.means$y[i,] = colMeans(founders$y[rows,,drop = F], na.rm = T)
      } # for(i)
      ...
      for(i in seq(1, length(missing), 2)) {
        ...
        founders$x[x.rows, curr.snps] =
                   matrix(colMeans(par.means$x[par[,i], curr.snps], na.rm = T),
                   length(x.rows), length(curr.snps), byrow = T)
        founders$y[x.rows, curr.snps] = 
                   matrix(colMeans(par.means$y[par[,i], curr.snps], na.rm = T),
                   length(x.rows), length(curr.snps), byrow = T)
      } # for(i)
