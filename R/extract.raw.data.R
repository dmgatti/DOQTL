...
extract.raw.data = function(in.path = ".", prefix, out.path = ".", 
  ...
  for(i in 1:length(in.path)) {
    ...
    samplefile = dir(path = in.path[i], pattern = "Sample_Map.txt", full.names = T)
    ...
    samples = read.delim(samplefile, stringsAsFactors = F)$Name

    # Find a file with "FinalReport" in the filename.
    rawfile = dir(path = in.path[i], pattern = "FinalReport", full.names = T)
