...
tabulate.geno = function(geno, founders, symbols) {
    ...
    tmp = array(0.01, c(length(symbols), length(founders$states), ncol(geno)),
    ...
    for(s in founders$states) {
      ...
      curr.geno = data.frame(geno[rows,, drop = F], row.names = NULL)
      ...
      tmp[,s,] = matrix(unlist(tbl), dim(tmp)[1], dim(tmp)[3])
      tmp[tmp == 0] = 0.01
      tmp[,s,] = tmp[,s,] / matrix(colSums(tmp[,s,]),
                 nrow = nrow(tmp[,s,]), ncol = ncol(tmp[,s,]), byrow = T)
    } # for(s)
