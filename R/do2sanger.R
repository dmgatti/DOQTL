...
# Sept. 7, 2013
################################################################################
# Arguments: do.files: characters vector of *.genotype.probs.Rdata files that 
...
do2sanger = function(do.files, snps, output.file = "do2sanger.txt", 
  ...
  for(c in 1:length(snps)) {
    ...
    con = file(description = paste("chr", curr.chr, ".", output.file, sep = ""),
          open = "w")
    writeLines(paste("ID", "CHR", "NCBI38.bp", "Ref", "Alt", 
...
#####       for all Sanger SNPs. 
    ...
    num.zero = rowMeans(geno == 0, na.rm = T)
    num.two  = rowMeans(geno == 2, na.rm = T)
