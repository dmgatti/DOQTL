...
#          analysis.
predict.sex = function(x, y, snps, plot = F) {
  ...
  x.int = rowMeans(x[,x.rng] + y[,x.rng], na.rm = T)

  y.rng = which(snps[,2] == "Y")
  y.int = rowMeans(x[,y.rng] + y[,y.rng], na.rm = T)
  ...
  mc = Mclust(cbind(x.int, y.int), G = 2, model = "EEE")
