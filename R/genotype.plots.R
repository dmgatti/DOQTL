...
chr.skeletons = function(chr, chrlen, ...) {
  ...
  plot(1, 1, col = 0, xlim = c(1, (2 * length(chrlen) + 1)),
       ylim = c(0, max(chrlen) + 10), xaxt = "n", yaxs = "i", xlab = "",
       ylab = "Mb")
  ...
  abline(h = 0:30 * 10, col = "grey80")
  abline(h = 0:5  * 50, col = "grey50", lwd = 1.2)
...
plot.genoprobs = function(x, snps, colors = "DO", chrlen = "mm10", ...) {
  ...
  usr = par("usr")
  legend(x = 28, y = usr[4], legend = colors[,2], col = colors[,3], pch = 15,
         bg = "white")
...
plot.genoprobs2 = function(x, snps, colors = "DO", chrlen = "mm10", ...) {
  ...
  # If chrlen is longer than one, then we assume that it is
  # a set of custom chr lengths. If the length is one, then we
  # assume that it is a genome specification (mm10 or rn6).
  genome = chrlen
  if(length(chrlen) == 1) {
    chrlen = get.chr.lengths(genome = chrlen)
  } # if(length(chrlen) == 1)
  ...
  if(!is.null(files)) {
    ...
    foreach(f = iter(files), .export = c("path", "snps"), .packages = "DOQTL") %dopar% {
      ...
      png(paste(sample, "genotype.probs.png", sep = "."), width = 1000,
          height = 1000, res = 144)
      plot.genoprobs(x = prsmth, snps = snps, main = sample)
      dev.off()
