...
html.report = function(path, qtl, perms, merge = F) {
  ...
  for(i in 1:length(qtl)) {
    ...
    tmp = create.page(path = path, qtl = qtl[[i]], pheno.name = names(qtl)[i],
          perms = perms)
  ...
  hwrite("QTL Summary", p, br = T, center = T, heading = 1)

  sig.qtl = sig.qtl[order(sig.qtl[,1]),]
  write.csv(sig.qtl, "sig.qtl.csv")
  hwrite("QTL Summary File", p, br = T, center = T, link = "sig.qtl.csv")
  ...
  hwrite(sig.qtl, p, br = T, row.bgcolor = "#aaaaaa")
...
create.page = function(path, qtl, pheno.name, perms) {
  ...
  hwrite(paste(pheno.name, "QTL"), p, br = T, heading = 1)
  ...
  if(!is.null(thr)) {
    ...
    png(paste(curr.path, image.file, sep = ""), width = 1000, height = 800,
        res = 128)
    qtl.plot(qtl, main = pheno.name, sig.thr = thr,
       sig.col = c("goldenrod", "orange", "red"))
    dev.off()

    hwriteImage(image.file, p, br = T, link = image.file)
    ...
    if(length(sig.A.chr) > 0 | length(sig.X.chr) > 0) {
      ...
      if(length(sig.A.chr) > 0) {
        ...
        for(c in 1:length(sig.A.chr)) {
          ...
          png(paste(curr.path, coef.image.files[c], sep = ""), width = 1000, 
              height = 800, res = 128)
          coef.plot(qtl, chr = sig.A.chr[c], main = pheno.name)
          dev.off()

          interval = bayesint(qtl$lod$A, chr = sig.A.chr[c], expandtomarkers = T)
      ...
      if(length(sig.X.chr) > 0) {
        ...
        if(all(paste("F", LETTERS[2:8], sep = ".") %in% colnames(qtl$coef$X))) {
          ...
          coef.plot(qtl, chr = "X", main = paste(pheno.name, "Females"), sex = "F")
          dev.off()

          interval = bayesint(qtl$lod$X, chr = "X", expandtomarkers = T)
          sig.tmp[1] = pheno.name
          sig.tmp[2:6] = interval[2,c(1:4,7)]
          sig.tmp[7] = mean(perms >= as.numeric(interval[2,7]))
          sig.tmp[8] = interval[2,5]
        ...
        if(all(paste("M", LETTERS[2:8], sep = ".") %in% colnames(qtl$coef$X))) {
          ...
          png(paste(curr.path, coef.image.files[length(coef.image.files)], 
              sep = ""), width = 1000, height = 800, res = 128)
          coef.plot(qtl, chr = "X", main = paste(pheno.name, "Males"), sex = "M")
          dev.off()
      ...
      for(i in 1:length(coef.image.files)) {
        hwriteImage(coef.image.files[i], p, br = T, link = coef.image.files[i])
      } # for(c)

    } #     if(length(sig.A.chr) > 0 | length(sig.X.chr) > 0)

  } else {
    qtl.plot(qtl$lod, main = pheno.name)
  } # else
