...
em.allele = function(data, founders, sex, snps, chr, trans.prob.fxn) {
  ...
  while(p <= maxIter & logLik - lastLogLik > epsilon) {
    ...
    for(i in 1:length(a)) {
      ...
      res = .C(filter_smooth_allele,
               dims = as.integer(c(dim(init.hmm$prsmth[,gen,,drop = F]), 4)),
    ...
    b = update.parameters.alleles(geno = data$geno, b = b, 
