...
estimate.cluster.params = function(founders, data, chr) {
  ...
  theta.means = aggregate(founders$theta, list(founders$code), mean)
  rho.means   = aggregate(founders$rho,   list(founders$code), mean)
...
#    rt = cbind(founders$theta[,s], founders$rho[,s])
#    dimnames(rt) = list(founders$code, c("t", "r"))
    mc = Mclust(rt, model = "VVI", prior = priorControl())
    ...
    if(any(prop.lt.min.prop)) {
      G = mc$G - sum(any(prop.lt.min.prop))
