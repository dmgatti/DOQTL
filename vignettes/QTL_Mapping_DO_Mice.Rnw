...
We will use example data from \href{http://www.ncbi.nlm.nih.gov/pubmed/22345611}{Svenson et.al, \textit{Genetics}, 2012}. Breifly, 149 mice (75 F, 74 M) were placed on either a chow (n = 100) or a high fat diet (n = 49). A  variety of clinical phenotypes were measured at two time points, roughly 14 weeks apart.  In this example, we will map the hemoglobin distribution width (HDW) at the second time point. We will load this data from the \href{http://bioconductor.org}{Bioconductor} data package \texttt{MUGAExampleData}.
