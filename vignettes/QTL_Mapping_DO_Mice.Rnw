...
We can also see how many unique SNP patterns are contained by the SNPs with LODs above 4.

<<label=uniquesdps,include=FALSE>>=
tmp[!duplicated(tmp$sdps),]
@
